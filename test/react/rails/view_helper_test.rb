...
class ViewHelperTest < ActionDispatch::IntegrationTest
  ...
  test 'ks_react_component accepts React props' do
    html = @helper.ks_react_component('Foo', {bar: 'value'})
    %w(data-react-class="Foo" data-react-props="{&quot;bar&quot;:&quot;value&quot;}").each do |segment|
      assert html.include?(segment)
    end
  end

  test 'ks_react_component accepts jbuilder-based strings as properties' do
    jbuilder_json = Jbuilder.new do |json|
      json.bar 'value'
    end.target!

    html = @helper.ks_react_component('Foo', jbuilder_json)
    %w(data-react-class="Foo" data-react-props="{&quot;bar&quot;:&quot;value&quot;}").each do |segment|
      assert html.include?(segment), "expected #{html} to include #{segment}"
    end
  end

  test 'ks_react_component accepts string props with prerender: true' do
    html = @helper.ks_react_component('Todo', {todo: 'render on the server'}.to_json, prerender: true)
  ...
  test 'ks_react_component passes :static to SprocketsRenderer' do
    html = @helper.ks_react_component('Todo', {todo: 'render on the server'}.to_json, prerender: :static)
    assert(html.include?('>render on the server</li>'), "it includes rendered HTML")
    assert(!html.include?('data-reactid'), "it DOESNT INCLUDE React properties")
  end

  test 'ks_react_component accepts HTML options and HTML tag' do
    assert @helper.ks_react_component('Foo', {}, :span).match(/<span\s.*><\/span>/)

    html = @helper.ks_react_component('Foo', {}, {:class => 'test', :tag => :span, :data => {:foo => 1}})
